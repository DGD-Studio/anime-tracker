...
const log = logger({ name: "Anime Tracker | InteractionCreate Event" })
...
events.interactionCreate = async (rawBot, interaction) => {
    ...
    if (interaction.data && interaction.id) {
        ...
        log.info(
            `[Command: ${bgYellow(black(String(interaction.data.name)))} - ${bgBlack(white(`Trigger`))
        ...
        if (command !== undefined) {
            if (interaction.data.name) {
                ...
                try {
                    ...
                    log.error(
                        `[Command: ${bgYellow(black(String(interaction.data.name)))} - ${bgBlack(red(`Error`))
                        }] by ${interaction.user.username}#${interaction.user.discriminator} in ${guildName}${guildName !== "Direct Message" ? ` (${guild.id})` : ``
                        }`,
                    );
